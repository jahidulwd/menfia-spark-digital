...
async function handle(request: Request) {
  ...
  const value = (data?.value ?? {}) as { key?: string; log?: { at: string; source: string; ok: boolean }[] };

  const provided = request.headers.get("x-keepalive-key");
  const matchesStoredKey = Boolean(value.key) && provided === value.key;
  if (!matchesStoredKey) {
